...
def calculate_workspace_hash(workspace_path: str) -> Optional[str]:
...
class ReviewGateServer:
        ...
        async def list_tools():
            ...
            tools = [
                Tool(
                    ...
                    description="Open Review Gate chat popup in Cursor for feedback and reviews. Use this when you need user input, feedback, or review from the human user. The popup will appear in Cursor and wait for user response for up to 60 minutes.",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "message": {
                            ...
                            "workspace_path": {
                                "type": "string",
                                "description": "当前工作区的绝对路径，用于多窗口支持。Agent应传递当前打开文件所在的工作区路径。",
                                "default": ""
                            }
                        }
                    }
    ...
    async def _handle_review_gate_chat(self, args: dict) -> list[TextContent]:
        """Handle Review Gate chat popup and wait for user input with 60 minute timeout"""
        ...
        # Get workspace_path from Agent arguments for multi-window support
        workspace_path = args.get("workspace_path", "")
        workspace_hash = calculate_workspace_hash(workspace_path) if workspace_path else None
        
        logger.info(f"🔑 Workspace hash for trigger: {workspace_hash or 'None (generic)'}")
        ...
        success = await self._trigger_cursor_popup_immediately({
            ...
            "timestamp": datetime.now().isoformat(),
            "immediate_activation": True,
            "workspace_path": workspace_path  # Pass through for multi-window support
        })
        
        if success:
            logger.info(f"🔥 POPUP TRIGGERED IMMEDIATELY - waiting for user input (trigger_id: {trigger_id}, hash: {workspace_hash or 'generic'})")
            ...
            # Wait for user input from the popup with 60 MINUTE timeout
            logger.info("⏳ Waiting for user input for up to 60 minutes...")
            user_input = await self._wait_for_user_input(trigger_id, timeout=3600, workspace_hash=workspace_hash)  # 60 MINUTE timeout
            ...
            if user_input:
                ...
                response = f"TIMEOUT: No user input received for review gate within 60 minutes"
                logger.warning("⚠️ Review Gate timed out waiting for user input after 60 minutes")
                return [TextContent(type="text", text=response)]
        else:
    ...
    async def _wait_for_user_input(self, trigger_id: str, timeout: int = 120, workspace_hash: str = None) -> Optional[str]:
        """Wait for user input from the Cursor extension popup with frequent checks and multiple response patterns"""
        # Build response patterns - prioritize workspace-specific files
        response_patterns = []
        
        # Primary: workspace-specific response file
        if workspace_hash:
            response_patterns.append(Path(get_temp_path(f"review_gate_response_{workspace_hash}_{trigger_id}.json")))
        ...
        logger.info(f"👁️ Monitoring for response files (hash: {workspace_hash or 'generic'}):")
                        ...
                        try:
                            ...
                            if file_content.startswith('{'):
                                ...
                                if response_trigger_id and response_trigger_id != trigger_id:
                                    ...
                                    # Clean up old response file to avoid repeated detection
                                    try:
                                        response_file.unlink()
                                        logger.info(f"🧹 Cleaned up old response file: {response_file}")
                                    except Exception as cleanup_error:
                                        logger.warning(f"⚠️ Could not clean old response file: {cleanup_error}")
                                    continue
    ...
    async def _trigger_cursor_popup_immediately(self, data: dict) -> bool:
        ...
        try:
            ...
            # Try to get workspace path for multi-window support
            # Priority: 1) Agent-provided path, 2) Environment variable
            workspace_path = data.get('workspace_path') or os.environ.get('CURSOR_WORKSPACE_PATH') or os.environ.get('WORKSPACE_PATH')
            workspace_hash = calculate_workspace_hash(workspace_path) if workspace_path else None
            
            # Determine trigger file path based on workspace hash
            if workspace_hash:
                trigger_file = Path(get_temp_path(f"review_gate_trigger_{workspace_hash}.json"))
                logger.info(f"📂 Workspace path: {workspace_path}")
                logger.info(f"🔑 Workspace hash: {workspace_hash}")
                logger.info(f"📄 Using workspace-specific trigger file: {trigger_file.name}")
            else:
                trigger_file = Path(get_temp_path("review_gate_trigger.json"))
                logger.info("📂 No workspace path available - using generic trigger file")
            ...
            trigger_data = {
                ...
                "immediate_activation": True,
                "workspace_path": workspace_path,  # For multi-window support
                "workspace_hash": workspace_hash   # Hash for file matching
            }
            
            logger.info(f"🎯 CREATING trigger file with data: {json.dumps(trigger_data, indent=2, ensure_ascii=False)}")
            
            # Write trigger file with immediate flush and UTF-8 encoding for proper Chinese/Unicode support
            ...
            # Note: Removed backup trigger files to prevent duplicate activations
    ...
    async def _create_backup_triggers(self, data: dict):
        ...
        try:
            # Get workspace path for multi-window support
            workspace_path = os.environ.get('CURSOR_WORKSPACE_PATH') or os.environ.get('WORKSPACE_PATH') or None
